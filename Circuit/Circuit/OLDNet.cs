...
namespace Circuit
{
    ...
    public class OLDNet<TVexValue, TArcValue>
    {
        ...
        protected delegate void Visit(int index, Action<int> action, FirstAdjVex firstAdjVex, NextAdjVex nextAdjVex);
        ...
        /// <param name="action">访问方法</param>
        protected void Traverse(Visit traverse, Action<int> action, FirstAdjVex firstAdjVex, NextAdjVex nextAdjVex)
        {
            ResetIsVisited();
        ...
        protected List<List<int>> GetRoute(int startVexID, int endVexID)
        {
            ...
            //遍历该点所有邻接点
            for (int adjVexID = FirstEdgeAdjVex(startVexID, out tempArc);
                adjVexID != -1; adjVexID = NextEdgeAdjVex(startVexID, tempArc, out tempArc))
            {
            ...
            while (route.Count > 0)
            {
                ...
                if (tempAdjVex.Count > 0)
                {
                    ...
                    //遍历该点所有邻接点
                    for (int adjVexID = FirstEdgeAdjVex(startVexID, out tempArc); adjVexID != -1; 
                        adjVexID = NextEdgeAdjVex(startVexID, tempArc, out tempArc))
                    {
                        //剔除路径里已有的点
